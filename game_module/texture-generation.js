...
export function generateTexture() {
    ...
    let canvasGray = document.createElement('canvas');
    canvasGray.id = 'texture-gray';
    canvasGray.width = 200;
    canvasGray.height = 200;
    document.body.appendChild(canvasGray);
    let ctxGray = canvasGray.getContext("2d");

    for (let x = 0; x < canvas.width; x++) {
        let g = Math.sin((x / canvas.width) * Math.PI) * 255;
        //console.log(g)
        //ctx.fillStyle = getRGBString(100, g, 100);

        for (let y = 0; y < canvas.height; y++) {
            let perlinNoiseValue = (perlinNoise(x * 0.01, y * 0.01) + 1) / 2;
            let perlinNoiseGrayscaleValue = perlinNoiseValue * 255;
            /**
             * Tutorial for procedural textures
             * http://www.upvector.com/?section=Tutorials&subsection=Intro%20to%20Procedural%20Textures
             */
            let sineNoisePattern = (Math.sin(0.75 * perlinNoiseGrayscaleValue) + 1) / 2;

            ctxGray.fillStyle = getGrayscaleRGBString(sineNoisePattern * 255);
            ctxGray.fillRect(x, y, 1, 1);

            let patternValue = sineNoisePattern * 50 + 10;
            let woodColor = "hsl(23, 60%, " + patternValue + "%)";
            ctx.fillStyle = woodColor;
            ctx.fillRect(x, y, 1, 1);
        }
    }

    let bumpMapTexture = new THREE.CanvasTexture(canvasGray); 
    let canvasTexture = new THREE.CanvasTexture(canvas);
    return { woodTexture: canvasTexture, bumpMapTexture: bumpMapTexture };
}
